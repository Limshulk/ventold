...
auto plugin_manager::load(std::string_view name, std::string_view path_prefix)
    ...
    log()->trace(
        "plugin_manager", "checking if plugin '{}' is already loaded.", name);
    if (_plugins.contains(name_str)) {
        log()->trace(
            "plugin_manager", "plugin '{}' is already loaded, skipping.", name);
...
auto plugin_manager::unload(std::string_view name) -> bool {
    ...
    if (it == _plugins.end()) {
        log()->debug(
            "plugin_manager", "plugin '{}' not found, cannot unload.", name);
        return false;
    }
